    ...
    detectionsOfThisFrame.forEach(function(itemDetected) {
      ...
      detectionsOfThisFrame.forEach(function(itemDetected) {
        if(mapOfItemsTracked.get(itemDetected.id)==null){
          undetectedItems.push(itemDetected);
        }
      });
      ...
      undetectedItems.forEach(function(item){
        ...
        mapOfItemsTracked.get(item.id)
                              .makeUnavailable()
                              .update(item, frameNb);
        mapOfItemsTracked.delete(item.id);
      });
    });
  } else {
    if(DEBUG_MODE) {
    ...
    mapOfItemsTracked.forEach(function(itemTracked) {
      ...
      detectionsOfThisFrame.forEach(function(itemDetected) {
        if(mapOfItemsTracked.get(itemDetected.id)==null){
          ...
          const itemTrackedPrevious= mapOfItemsTracked.get(itemDetected.id);
          mapOfItemsTracked.set(itemDetected.id, itemDetected); // update 
          var itemTrackedNow = mapOfItemsTracked.get(itemDetected.id);  
          // itemTracked.countDown(frameNb);
          itemTracked.updateTheoricalPositionAndSize();
          mapOfItemsTracked.get(itemTracked.id)
                              .makeUnavailable()
                              .update(itemTracked, frameNb)
        }
      });
    });
  } */
