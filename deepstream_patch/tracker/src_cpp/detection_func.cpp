...
char *tracker_data_to_json(NvDsFrameMeta *frame_meta)
{
    ...
    for (NvDsMetaList * l_obj = frame_meta->obj_meta_list; l_obj != NULL;
      ...
      char *buf = (char *)calloc(2048, sizeof(char));
      sprintf(buf, "  { \"id\":%d, \"class_id\":%d, \"name\":\"%s\", \"relative_coordinates\":{\"center_x\":%f, \"center_y\":%f, \"width\":%f, \"height\":%f}, \"confidence\":%f} ,",
          obj->object_id, obj->class_id, obj->obj_label, bbox[0], bbox[1], bbox[2], bbox[3], obj->confidence);
